import ScenarioDetailShell from "@/components/scenarios/scenario-detail-shell";
import { requireRole } from "@/features/auth/requireRole";
import { APP_ROLES } from "@/lib/auth/roles";
import { logMeasuredOperation } from "@/lib/observability/performance";
import { getScenarioDetail } from "@/lib/scenarios/queries";
...
async function getScenarioDetailPageData({ params }: Props) {
  ...
  try {
    ...
    logMeasuredOperation({
      operation: "page.scenarios.detail.GET",
      durationMs: Date.now() - startedAt,
      records,
      status,
      meta: {
        path: "/[locale]/scenarios/[slug]",
        slug: slugForLog,
        lazyRelated: true,
      },
    });
...
export default async function ScenarioDetailPage(props: Props) {
  const { locale, scenario } = await getScenarioDetailPageData(props);
  ...
  return (
    <main className="relative min-h-screen bg-[#f5f5f3] pb-20 transition-all duration-300">
      ...
      <div className="relative mx-auto max-w-360 px-4 pt-10 sm:px-6 lg:px-8 transition-all duration-300">
        <ScenarioDetailShell locale={locale} scenario={scenario} />
      </div>
    </main>
  );
}
