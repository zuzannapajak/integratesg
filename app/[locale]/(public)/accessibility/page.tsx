...
export default async function AccessibilityPage({ params }: Props) {
  ...
  const t = await getTranslations({ locale, namespace: "PublicContent.Accessibility" });

  const overviewItems = [1, 2, 3, 4, 5].map((index) => t(`overview.items.${index}`));
  const accessibilityMeasures = [1, 2, 3, 4].map((index) => t(`sections.2.items.${index}`));

  const textSections = [1, 3, 4, 5].map((index) => ({
            ...
            <div className="space-y-10">
              ...
              <section>
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                  {accessibilityMeasures.map((item) => (
                    <div
                      key={item}
